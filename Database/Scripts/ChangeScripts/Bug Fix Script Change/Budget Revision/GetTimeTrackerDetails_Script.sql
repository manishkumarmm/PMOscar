IF EXISTS(SELECT * FROM sys.objects WHERE type = 'P' AND name = 'GetTimeTrackerDetails')
     DROP PROCEDURE GetTimeTrackerDetails
     
USE [PMOscar_Dev]
GO

/****** Object:  StoredProcedure [dbo].[GetTimeTrackerDetails]    Script Date: 08/10/2016 10:52:45 ******/
SET ANSI_NULLS ON
GO
 ...
 (
	select EmployeeCode, ISNULL(
		(CASE WHEN LeaveFromDate != LeaveToDate THEN 
		 ...
		 ) as a  where LeaveFromDate between @dayfrom and @dayto or  LeaveToDate between @dayfrom and @dayto order by EmployeeCode
