IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'getProjectDashboardDetails') AND type in (N'P', N'PC'))
...
--Modified On : 2018-06-15
--Modified By : Vibin MB
--Modified On : 2018-06-19
-- ==============================================================================
...
if (@Status='Create')                                    
    ...
    begin                              
      Select distinct A.projectID,B.PhaseID,A.ProjectName,B.BillableHours,B.BudgetHours,B.RevisedBudgetHours,B.EstimationRoleID,                           
					 estRole.RoleName as EstRoleName,estRole.ShortName as EstRoleshortName,D.Role,D.ShortName,D.EstimationPercentage,                                            
					  CASE A.ProjectType                                 
						WHEN  'F'  THEN cast(isnull(E.ActualHours,0)as decimal(10,2)) * D.EstimationPercentage  ELSE cast(isnull(E.ActualHours,0)as decimal(10,2))                                                      
					 ...
					 CASE A.ProjectType                                 
						WHEN  'F'  THEN cast(isnull((F.ActualHours * D.EstimationPercentage),0)as decimal(10,2)) ELSE cast(isnull((F.ActualHours),0)as decimal(10,2))                                               
						END ActualHours
					 ,B.Comments as RevisedComments ,B.ProjectEstimationID                       
					 from Project A 
					 ...
					 left join (
								  Select cast(isnull(sum(A.ActualHours),0)as decimal(10,2))ActualHours,isnull(sum(A.EstimatedHours),0)EstimatedHours,                                             
					 ...
					 left join (
								  Select cast(isnull(sum(A.ActualHours),0)as decimal(10,2))ActualHours,A.phaseID,A.RoleID,A.ProjectId from TimeTracker A where 1=1                                    
								  group by A.RoleID,A.phaseID,A.ProjectId
					...
					D.EstimationPercentage  ,E.EstimatedHours,E.ActualHours ,F.ActualHours, B.Comments,B.ProjectEstimationID order by A.ProjectName                        
				...
				BEGIN                                   
					IF(@IsIncludeProposalAndVAS<>1)
						BEGIN 
							...
							,CASE WHEN E.ActualHrs>E.BudgetHours THEN 'R' ELSE 'G' END AS BudgetStatus,isnull(E.PeriodEstimetedHrsTotal,0) Estimate,cast(isnull(E.PeriodActualHrsTotal,0)as decimal(10,2))SortOrder                       
							FROM  Dashboard db                              
							...
							LEFT JOIN
								(
								  ...
								  cast(sum(ISNULL(ActualHrs,0))as decimal(10,2)) ActualHrs ,cast(sum(PeriodActualHrsTotal)as decimal(10,2))PeriodActualHrsTotal,sum(round(PeriodEstimetedHrsTotal,0))PeriodEstimetedHrsTotal
								  FROM
									(SELECT dt1.ProjectID,dt1.PhaseID,sum(dt1.BudgetHours) BudgetHours,sum(dt1.RevisedBudgetHours) RevisedBudgetHours,
									 sum(dt1.ActualHrs) ActualHrs ,cast(sum(dt1.PeriodActualHrsTotal)as decimal(10,2))PeriodActualHrsTotal,sum(round(dt1.PeriodEstimetedHrsTotal,0))PeriodEstimetedHrsTotal                
									 FROM(	
											SELECT  projest.EstimationRoleID,projest.PhaseID,projest.ProjectID ,avg(ISNULL(projest.BudgetHours,0)) BudgetHours,
											avg(ISNULL(projest.RevisedBudgetHours,0)) RevisedBudgetHours,cast(sum(ISNULL(projest.ActualHrs,0))as decimal(10,2)) ActualHrs,                                    
											cast(sum(projest.PeriodActualHrsAdjusted)as decimal(10,2))PeriodActualHrsTotal,sum(round(projest.PeriodEstimetedHrsAdjusted,0))PeriodEstimetedHrsTotal  
		...
		BEGIN    
			...
			cast(isnull(dt3.PeriodActualHrsAdjusted,0)as decimal(10,2))SortOrder                            
			FROM  Dashboard db                              
			JOIN dbo.ProjectDashboard A ON A.DashboardID=db.DashboardID AND db.DashboardID=@DashboardID                      
			INNER JOIN [User] B ON A.ProjectOwner=B.UserID                                    
			INNER JOIN [User] C ON A.ProjectManager=C.UserID                                   
			...
			LEFT JOIN
				(
				  SELECT ProjectID,sum(ISNULL(BudgetHours,0)) BudgetHours,sum(ISNULL(RevisedBudgetHours,0)) RevisedBudgetHours,
				  cast(sum(ISNULL(ActualHrs,0))as decimal(10,2)) ActualHrs ,cast(sum(PeriodActualHrsTotal)as decimal(10,2))PeriodActualHrsTotal
				  FROM
					(SELECT dt1.ProjectID,dt1.PhaseID,sum(dt1.BudgetHours) BudgetHours,sum(dt1.RevisedBudgetHours) RevisedBudgetHours,
					cast(sum(dt1.ActualHrs)as decimal(10,2)) ActualHrs ,cast(sum(dt1.PeriodActualHrsTotal)as decimal(10,2))PeriodActualHrsTotal               
					 FROM(	
							SELECT  projest.EstimationRoleID,projest.PhaseID,projest.ProjectID ,avg(ISNULL(projest.BudgetHours,0)) BudgetHours,
							avg(ISNULL(projest.RevisedBudgetHours,0)) RevisedBudgetHours,cast(sum(ISNULL(projest.ActualHrs,0))as decimal(10,2)) ActualHrs,                                    
							cast(sum(projest.PeriodActualHrsAdjusted)as decimal(10,2))PeriodActualHrsTotal  
							FROM ProjectDashBoardEstimation projest             
							WHERE DashBoardId =@DashboardID AND PhaseId NOT IN (1,6) --EXCLUDED PROPOSAL & VAS         
							GROUP BY projest.EstimationRoleID,projest.PhaseID,projest.ProjectID            
						  ) AS dt1            
    ...
    BEGIN   
		...
		BEGIN                           
			 ...
			 dt3.ActualHrsTotal, isnull(dt4.PeriodEstimatedTotal,0) Estimate,cast(isnull(dt4.PeriodActualHrsAdjusted,0)as decimal(10,2))Actual                      
			 FROM                                     
			 (
			...
			LEFT JOIN                                     
			(SELECT cast(sum(isnull(projest.ActualHrsAdjusted,0))as decimal(10,2))ActualHrsTotal,projest.ProjectID  
		...
		BEGIN
			 ...
			 cast(isnull(dt3.PeriodEstimatedTotal,0)as decimal(10,2)) Estimate,cast(isnull(dt3.PeriodActualHrsTotal,0)as decimal(10,2))Actual                        
			 FROM                                     
			(SELECT  A.projectID, A.ProjectName,B.PhaseID,d1.BillableHours,d1.BudgetHours,d1.RevisedBudgetHours,
			 cast(sum(isnull(B.ActualHrsAdjusted,0))as decimal(10,2)) ActualHours,d.[status] AS DashboardStatus                                
			 FROM                          
			...
			LEFT JOIN                                     
			(SELECT cast(sum(isnull(projest.ActualHrsAdjusted,0))as decimal(10,2))ActualHrsTotal,projest.ProjectID ,
			 cast(sum(projest.PeriodActualHrsAdjusted)as decimal(10,2))PeriodActualHrsTotal,	sum(round(projest.PeriodEstimetedHrsAdjusted,0)) PeriodEstimatedTotal  
			 FROM dbo.ProjectDashboardEstimation projest 
    ...
    /* PeriodEffortPhase starts here */
     BEGIN   
		...
		BEGIN 
			...
			cast(sum(B.PeriodActualHrsAdjusted)as decimal(10,2)) AS PeriodActualHrs,cast(isnull(D.PeriodEstHrsTotal,0)as decimal(10,2))PeriodEstHrsTotal,
			cast(isnull(D.PeriodActualHrsTotal,0)as decimal(10,2))PeriodActualHrsTotal,cast(isnull(dt3.PeriodActualHrsAdjusted,0)as decimal(10,2))SortOrder                                  
			FROM Project proj                                  
			...
			LEFT JOIN 
				(SELECT B.ProjectID,sum(B.PeriodEstimetedHrsAdjusted)PeriodEstHrsTotal,
				cast(sum(B.PeriodActualHrsAdjusted)as decimal(10,2))PeriodActualHrsTotal                                    
			...
			LEFT JOIN                                     
				(                                    
					SELECT projest.ProjectID,cast(sum(projest.PeriodActualHrsAdjusted)as decimal(10,2))PeriodActualHrsAdjusted  FROM                                     
					ProjectDashboardEstimation projest WHERE projest.DashboardID =@DashboardID	 
					And projest.PhaseID NOT IN(1,6) GROUP BY projest.ProjectID                                    
		...
		ELSE
			BEGIN
				/* This block includes Proposal and VAS phase values (PhaseID=1 and 6) */
					SELECT DISTINCT Ph.Phase AS CurrPhase ,A.projectID,A.ProjectName,C.Phase,B.PhaseID,
				sum(round(B.PeriodEstimetedHrsAdjusted,0)) AS PeriodEstimetedHrs,      
				cast(sum(B.PeriodActualHrsAdjusted)as decimal(10,2))PeriodActualHrs,isnull(D.PeriodEstHrsTotal,0)PeriodEstHrsTotal,
				cast(isnull(D.PeriodActualHrsTotal,0)as decimal(10,2))PeriodActualHrsTotal,cast(isnull(D.PeriodActualHrsAdjusted,0)as decimal(10,2))SortOrder                                  
				FROM  Project proj                                      
				LEFT JOIN ProjectDashboard A ON proj.ProjectID=A.ProjectID 
				LEFT JOIN dbo.ProjectDashboardEstimation B ON A.ProjectID=B.ProjectID AND A.DashboardID=B.DashboardID  
				LEFT JOIN Phase C ON B.PhaseID=C.PhaseID                                   
				...
				LEFT JOIN 
					...
					cast(sum(B.PeriodActualHrsAdjusted)as decimal(10,2))PeriodActualHrsTotal,
					cast(sum(B.PeriodActualHrsAdjusted)as decimal(10,2))PeriodActualHrsAdjusted                                     
					 FROM ProjectDashboardEstimation B 
     ...
     begin  
		...
		BEGIN 
			...
			isnull(dt3.PeriodEstimatedTotal,0) Estimate,cast(isnull(dt3.PeriodActualHrsTotal,0)as decimal(10,2))SortOrder                                  
			from                                     
			(
				...
				(                  
				 ...
				 (                                    
				  ...
				  left join                                     
				  (                                    
						Select cast(sum(isnull(projest.ActualHrsAdjusted,0))as decimal(10,2))ActualHrsTotal,projest.ProjectID,cast(sum(projest.PeriodActualHrsAdjusted)as decimal(10,2))PeriodActualHrsTotal,
						sum(round(projest.PeriodEstimetedHrsAdjusted,0)) PeriodEstimatedTotal  from                                     
   ...
   ELSE IF (@tabType='PeriodEffortRole')                                    
     BEGIN                                    
		...
		BEGIN                         
			/* This block INCLUDES Proposal and VAS phase values */         
            SELECT DISTINCT dt1.projectID,dt1.ProjectName,dt1.EstimationRoleID,dt1.RoleName                                    
			,dt1.PeriodEstimetedHrs,dt1.PeriodActualHrs,dt3.EstTotal ,dt3.ActTotal,cast(isnull(dt3.PeriodActualHrsAdjusted,0)as decimal(10,2))SortOrder                                    
			FROM Project prjt 
			LEFT JOIN	(
					SELECT A.projectID,A.ProjectName,B.EstimationRoleID,estRole.RoleName                                    
					,SUM(ROUND(B.PeriodEstimetedHrsAdjusted,0))  PeriodEstimetedHrs,cast(SUM(B.PeriodActualHrsAdjusted)as decimal(10,2)) PeriodActualHrs                                    
					FROM
			...
			LEFT JOIN
			(                                    
				SELECT cast(SUM(ISNULL(projest.PeriodActualHrsAdjusted,0))as decimal(10,2))ActTotal,projest.ProjectID,
				sum(round(projest.PeriodEstimetedHrsAdjusted,0))EstTotal,cast(SUM(projest.PeriodActualHrsAdjusted)as decimal(10,2)) PeriodActualHrsAdjusted from                                     
				dbo.ProjectDashboardEstimation projest WHERE projest.DashboardID =@DashboardID   group by projest.ProjectID                        
			) AS dt3 ON dt3.ProjectID=dt1.ProjectID            
		...
		BEGIN
			/* This block EXCLUDES Proposal and VAS phase values (PhaseID=1 and 6) */         
			SELECT DISTINCT dt1.projectID,dt1.ProjectName,dt1.EstimationRoleID,dt1.RoleName                                    
			,dt1.PeriodEstimetedHrs,dt1.PeriodActualHrs,dt3.EstTotal ,dt3.ActTotal,cast(isnull(D.PeriodActualHrsTotal,0)as decimal(10,2))SortOrder,
			...
			cast(sum(PDE.PeriodActualHrsAdjusted)as decimal(10,2))PeriodActualHrsTotal,
