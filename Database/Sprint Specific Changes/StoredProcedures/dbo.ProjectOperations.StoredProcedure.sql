IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'ProjectOperations') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[ProjectOperations]
/****** Object:  StoredProcedure [dbo].[ProjectOperations]    Script Date: 4/19/2018 1:02:16 PM ******/
SET ANSI_NULLS ON
GO
				...
				BEGIN                   
					IF not exists(select *,ProjectName from [dbo].Project where ProjectCode = case when @ProjectCode = '' then '%' else @ProjectCode end and  ProjectId != @ProjectId)                          
						Begin         
							...
							SVNPath = @SVNPath,ProjectDetails = @ProjectDetails,WorkOrderID = @WorkOrderID,BugProjectId=@BugzillaProjectId
						...
						IF not exists(select *,ProjectName from [dbo].Project where ProjectCode = case when @ProjectCode = '' then '%' else @ProjectCode end)                          
							Begin          
							   ...
							   (                          
								 ...
								 UpdatedDate,ProjStartDate,MaintClosingDate,POComments,DevURL,DemoURL,QaURL,ProductionURL, ClientId,ProgId,Utilization,CostCentreID, 
								 ShowInDashboard,SVNPath,ProjectDetails,WorkOrderID,BugProjectId
							   )                               
							   Values                              
							  (                          
								...
								@ProductionURL,@ClientId,@ProgramId,@Utilization,@CostCentreID ,@ShowInDashboard,@SVNPath, @ProjectDetails,@WorkOrderID,@BugzillaProjectId               
		...
		Begin   
			...
			if @ProjectCode='1'                      
				begin     
					...
					sum(cast(pe.BillableHours AS BIGINT))BillableHours,sum(cast(pe.BudgetHours AS BIGINT))BudgetHours,                         
					sum(cast(pe.RevisedBudgetHours AS BIGINT))RevisedBudgetHours,isnull(D.ActualHours,0)ActualHours, prj.WorkOrderID
					From Project prj                 
					...
					left join (SELECT CASE P.ProjectType  WHEN  'F'  THEN isnull(sum(A.ActualHours* B.EstimationPercentage),0)
							 ELSE isnull(sum(A.ActualHours),0) END ActualHours,    
					...
					ph.Phase, prj.Utilization,D.ActualHours  ,C.ClientName,P.ProgName,prj.WorkOrderID Order by  ProjectName                       
				end                       
	 ...
	 If @OpMode = 'SELECTALL'                        
		Begin                                               
			Select ProjectName,ShortName,ProjectType,ProjectOwner,ProjectManager,DeliveryDate,RevisedDeliveryDate ,PhaseID,                        
				   ...
				   DemoURL,QaURL,ProductionURL,ClientId,ProgId,ISNULL(Utilization,0) AS Utilization,CostCentreID ,ShowInDashboard,
				   SVNPath,ProjectDetails,WorkOrderID,BugProjectId 
				   From Project prj Where ProjectId = @ProjectId   Order by  ProjectName                                               
		End 
