IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'GetRevisedBudgetList') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[GetRevisedBudgetList]
SET ANSI_NULLS ON
GO
...
--Modified by : Deepa R
--Modified Date :2019-03-07
--Changes Done : New design for the Budget revisionLog Page.
-- =============================================

CREATE Procedure [dbo].[GetRevisedBudgetList]     
 @ProjectID  int,
 @UserID int
 ...
 BEGIN 
		...
		Begin 
				...
				LEFT Join 
					...
					Where B.ProjectID=@ProjectID  and  ISNULL(B1.IsReallocated, 0 ) <>1 									
					GROUP BY 
					B.ProjectID
							  ,B.BudgetRevisionID
							  ,B.[BudgetRevisionName]
					  ...
					  LEFT JOIN
						...
						(
						 ...
						 WHERE  BL1.Status='Approved'  
							...
							where   ISNULL(B1.IsReallocated, 0 ) <>1 					
